...
import { Plus, X, Upload, FileText, ArrowLeft, Loader2, Calendar, Eye, Image as ImageIcon, Search, Trash2, UserPlus, Pencil, ExternalLink } from "lucide-react";
import { toast } from "sonner";
import { productExtraInfoApi, ProductExtraInfo, ProductExtraInfoFile } from "@/lib/api/product-extra-info";
...
export const ProductExtraInfoTab: React.FC = () => {
    ...
    const [editingInfo, setEditingInfo] = useState<ProductExtraInfo | null>(null);
    ...
    const [existingImages, setExistingImages] = useState<ProductExtraInfoFile[]>([]);
    const [existingPdfs, setExistingPdfs] = useState<ProductExtraInfoFile[]>([]);
    ...
    const openEditForm = async (info: ProductExtraInfo) => {
        setLoadingDetail(true);
        try {
            const res = await productExtraInfoApi.getById(info.id);
            if (res.success && res.data) {
                const full = res.data;
                setEditingInfo(full);
                setTitle(full.title);
                setBody(full.body || "");
                setExistingImages(full.image_files || []);
                setExistingPdfs(full.pdf_files || []);
                setImages([]);
                setImagePreviews([]);
                setPdfs([]);
                setError("");
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            const response = await productExtraInfoApi.create({ title, body, images, pdfs });
            if (response.success) {
    ...
    const handleUpdate = async () => {
        ...
        setIsSaving(true);
        try {
            const response = await productExtraInfoApi.update(editingInfo.id, {
                title,
                body,
                existingImages,
                existingPdfs,
            ...
            if (response.success) {
                toast.success("Extra Information package updated successfully!");
                setIsFormOpen(false);
                resetForm();
                fetchInfos();
                if (viewingInfo?.id === editingInfo.id && response.data) {
            ...
            <div className="p-6">
                <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center gap-4">
                        ...
                        <h2 className="text-2xl font-bold">
                            {isEditing ? "Edit Product Extra Information" : "Add Product Extra Information"}
                        </h2>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow space-y-6">
                    ...
                    <div>
                        ...
                        <RichTextEditor content={body} onChange={setBody} placeholder="Enter body content here..." />
                    </div>
                    ...
                    <div>
                        ...
                        {isEditing && existingImages.length > 0 && (
                            <div className="mb-4">
                                <p className="text-xs text-gray-500 mb-2">Existing images (remove to delete on save):</p>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">PDF Documents</label>

                        {isEditing && existingPdfs.length > 0 && (
                            <div className="mb-4 space-y-2">
                                <p className="text-xs text-gray-500">Existing PDFs (remove to delete on save):</p>
                                {existingPdfs.map((pdf, index) => (
                                    <div key={pdf.url} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                        ...
                        <div className="flex items-center justify-center w-full">
                            <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                                ...
                                <input type="file" className="hidden" multiple accept=".pdf,application/pdf" onChange={handlePdfUpload} />
                            </label>
                        </div>
        ...
        return (
            <div className="p-6 space-y-6">
                <div className="flex items-center justify-between gap-4">
                    ...
                    <button
                        onClick={() => openEditForm(viewingInfo)}
                        className="inline-flex items-center gap-2 text-sm px-4 py-2 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 rounded-md transition-colors"
                    >
                ...
                <>
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1">
                                Manage extra details, Markdown bodies, and attached files for products.
                            </p>
                        </div>
                        <button
                            onClick={openAddForm}
                        ...
                        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                            <div className="bg-white rounded-lg p-6 w-full max-w-md">
                                ...
                                <p className="text-sm text-gray-600 mb-4">
                                    Send the extra info package "
                                    <span className="font-semibold">{selectedInfo.title}</span>" to a user via email.
                                </p>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500 mt-1">
                                            This will be used in the email subject and body, instead of the Extra Info title.
                                        </p>
                                    </div>
                                </div>
