...
export const OrdersTab: React.FC = () => {
    ...
    const saveTrackingInfo = async (orderId: string, newStatus?: string) => {
        try {
            ...
            if (newStatus) {
                ...
                if (newStatus === 'delivered' && !orderDetails?.order?.delivered_at) {
                    payload.delivered_at = new Date().toISOString();
                }
            } else if (trackingForm.tracking_number && !orderDetails?.order?.shipped_at) {
                // Auto-set status to shipped if tracking is added and it wasn't shipped before
            ...
            toast.success(newStatus ? `Order marked as ${newStatus}` : "Tracking information updated successfully!");
                                    ...
                                    <tr className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            <div className="text-xs text-gray-500 flex items-center gap-1.5 mt-0.5">
                                                {order.user_email}
                                                <a href={`mailto:${order.user_email}`} className="text-gray-400 hover:text-blue-600 transition-colors" title="Email customer">
                                                ...
                                                {order.status === "paid" && (
                                                    <button
                                                        onClick={() => {
                                                            ...
                                                            // Scroll to tracking section or focus it
                                                            setTimeout(() => {
                                                                const el = document.getElementById(`tracking-section-${order.id}`);
                                                                ...
                                                                <button
                                                                    onClick={() => {
                                                                        if (!trackingForm.tracking_number) {
                                                                            toast.error("Please provide a tracking number before dispatching.");
                                                                            return;
                                                                        }
                                                                ...
                                                                <div className="space-y-2 text-sm text-gray-700">
                                                                    ...
                                                                    {orderDetails.order?.phone && (
                                                                        <div className="flex items-center gap-2 text-gray-600 mt-1">
                                                                            ...
                                                                            <span className="truncate">{orderDetails.order.phone}</span>
                                                                            <span className="text-gray-300">|</span>
                                                                            <a href={`tel:${orderDetails.order.phone}`} className="text-blue-600 hover:text-blue-800 text-xs font-medium hover:underline shrink-0">
                                                                                Call
                                                                            </a>
                                                                            <a href={`https://wa.me/${orderDetails.order.phone.replace(/[^+\d]/g, '')}`} target="_blank" rel="noopener noreferrer" className="text-emerald-600 hover:text-emerald-800 text-xs font-medium hover:underline ml-1 shrink-0">
                                                                                WhatsApp
                                                                            </a>
                                                                        </div>
                                                                    )}
                                                                    {orderDetails.order?.email && (
                                                                        <div className="flex items-center gap-2 text-gray-600 mt-1">
                                                                            ...
                                                                            <span className="truncate">{orderDetails.order.email}</span>
                                                                            <span className="text-gray-300">|</span>
                                                                            <a href={`mailto:${orderDetails.order.email}`} className="text-blue-600 hover:text-blue-800 text-xs font-medium hover:underline shrink-0">
                                                                                Email
                                                                            </a>
                                                                        </div>
                                                                    )}
